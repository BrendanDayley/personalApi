...
app.get('/hobbies', function(req, res){
	...
	res.send(JSON.stringify({hobbies: hobbies}));
}
});
app.get('/occupations', function(req, res){
	...
	res.send(JSON.stringify({Occupations: occupations}));
	}
});
app.get('/occupations/latest', function(req, res){
	res.send(JSON.stringify({mostRecentOccupation: occupations[occupations.length - 1]}))
});
